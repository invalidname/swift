...
public:
  ...
  IndexSubset *deserializeaDifferentiableParamIndices();
...
IndexSubset *DeclDeserializer::deserializeaDifferentiableParamIndices() {
  using namespace decls_block;
  ...
  while (true) {
    ...
    if (recordID == ERROR_FLAG) {
      ...
      case decls_block::Differentiable_DECL_ATTR: {
        ...
        auto *indices = deserializeaDifferentiableParamIndices();
        if (!indices)
          return MF.diagnoseFatal();
