...
opened, the user is unknown, so the state of the app could be called _unregistered_.
After the user has registered or logged into an account, the state is _logged in_.
After some time of inactivity, the user’s session may expire, leaving the app in
a _session expired_ state.
...
![A diagram showing the states of the app: unregistered, logged in, and session expired.](maintaining-state-in-your-apps-1)
