...
Creates a units format style using the given parameters, from a type method.

Use this convenience factory function in situations that expect a ``Swift/Duration/UnitsFormatStyle``, such as ``Swift/Duration/formatted(_:)``, as an alternative to using the full initializer.
